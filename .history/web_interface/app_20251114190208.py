...
def update_dashboard(_n_intervals, _refresh_clicks, _load_clicks, _existing_figure, symbol):
    ...
    try:
        ...
        symbol = "BTC/USDT" if not symbol or symbol.strip() == "" else symbol.strip().upper()
        ...
        try:
            ...
            # Используем те же данные для volume profile
            volume_fig = create_volume_profile_chart(dataframe=dataframe)
        except ImportError:
