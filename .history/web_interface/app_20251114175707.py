...
def update_dashboard(n_intervals, refresh_clicks, existing_figure):
    ...
    try:
        ...
        fallback_fig = go.Figure().add_trace(go.Scatter(x=[1], y=[1], name="Error"))
    ...
    try:
        if active_tab == "footprint":
            try:
                ...
                return html.Div([dbc.Alert("Footprint компонент не найден", color="info")])
            ...
            try:
                ...
                return html.Div([dbc.Alert("Market Profile компонент не найден", color="info")])
            ...
            try:
                ...
                return html.Div([dbc.Alert("Confluence компонент не найден", color="info")])
            ...
            try:
                ...
                return html.Div([dbc.Alert("Backtest компонент не найден", color="info")])
            ...
            try:
                ...
                return html.Div([dbc.Alert("Signals компонент не найден", color="info")])

    except Exception as e:
        logger.error(f"Error in tab content: {e}", exc_info=True)
        return html.Div([dbc.Alert(f"Error loading {active_tab}: {e!s}", color="danger")])
...
if __name__ == "__main__":
    ...
    # Запускаем открытие браузера в фоне (если не запущено через run.py)
