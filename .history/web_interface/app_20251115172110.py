...
if __name__ == '__main__':
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Ошибка инициализации системы сигналов: {e}", exc_info=True)
        ...
        try:
            ...
            if telegram_bot:
                ...
                if telegram_bot.is_running:
                    logger.info("✅ Telegram бот успешно запущен: t.me/MaxFlash_bot")
                    print("\n✅ Telegram бот запущен: t.me/MaxFlash_bot")
                else:
                    logger.warning("⚠️ Telegram бот не запустился (is_running=False)")
                    print("\n⚠️ Telegram бот не запустился. Проверь логи.")

                # Интегрируем бота с монитором для отправки уведомлений
                if market_monitor:
                    market_monitor.telegram_bot = telegram_bot
            else:
                logger.error("❌ Не удалось создать Telegram бота (get_telegram_bot вернул None)")
        ...
        logger.error(f"❌ Не удалось импортировать модули Telegram бота: {e}")
        print(f"\n❌ Ошибка импорта: {e}")
