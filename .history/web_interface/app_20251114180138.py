...
try:
    ...
    FreqtradeClient = None  # type: ignore
...
def update_dashboard(_n_intervals, _refresh_clicks, _existing_figure):
    ...
    try:
        ...
        fallback_fig = go.Figure().add_trace(
            go.Scatter(x=[1], y=[1], name="Error")
        )
        fallback_fig.update_layout(
            template="plotly_dark", title="Error"
        )
...
def update_tab_content(active_tab):
    ...
    try:
        ...
        return html.Div([
            dbc.Alert(f"Error loading {active_tab}: {str(e)}",
                      color="danger")
        ])
