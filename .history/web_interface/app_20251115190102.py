...
def update_dashboard(
    ...
    try:
        ...
        try:
            ...
            # Получаем реальные сигналы для текущего символа и активные
            real_signals = signals_service.get_all_signals_for_dashboard(symbol)
            
            # Если есть сигналы, передаем их в компонент
            if real_signals:
                signals_result = create_signals_panel(signals=real_signals)
            else:
                # Если сигналов нет, пытаемся получить хотя бы для текущего символа
                symbol_signals = signals_service.get_signals_for_symbol(symbol)
                if symbol_signals:
                    signals_result = create_signals_panel(signals=symbol_signals)
                else:
                    signals_result = create_signals_panel()  # Тестовые данные
            
            # Проверяем тип результата
            if isinstance(signals_result, (html.Div, list)):
                signals = signals_result
        except ImportError as e:
            logger.warning("Не удалось загрузить компоненты сигналов: %s", str(e))
        except Exception as e:
            logger.error("Ошибка получения сигналов: %s", str(e), exc_info=True)
