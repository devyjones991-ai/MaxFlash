...
def fetch_market_data(
    symbol_name: str, timeframe: str = "15m", limit: int = 200
):
    """
    Загружает рыночные данные для указанного символа.

    Args:
        symbol_name: Торговая пара (например, BTC/USDT)
    ...
    try:
        ...
        try:
            ...
            # Загружаем данные
            ohlcv = exchange.fetch_ohlcv(symbol_name, ccxt_tf, limit=limit)
            ...
            logger.info(
                "Загружены данные для %s: %s свечей",
                symbol_name, len(df)
            )
            return df
        ...
        np.random.seed(hash(symbol_name) % 1000)
        ...
        base_price = base_prices.get(symbol_name, 1000)

        price_change = base_price * 0.01
        prices = base_price + np.cumsum(
            np.random.randn(limit) * price_change
        )
        ...
        logger.info("Сгенерированы тестовые данные для %s", symbol_name)
        return df
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    dcc.Interval(
                        ...
                        # 30 секунд (увеличено для производительности)
                        interval=30*1000,
                        n_intervals=0,
                        disabled=False
                    )
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    dcc.Interval(
                        ...
                        # Обновление каждые 15 секунд (увеличено)
                        interval=15*1000,
                        n_intervals=0,
                        disabled=False
                    ),
            ...
            dbc.Card([
                ...
                dbc.CardBody([
                    ...
                    dcc.Interval(
                        ...
                        # Проверка каждые 10 секунд (увеличено)
                        interval=10*1000,
                        n_intervals=0,
                        disabled=False
                    )
...
def update_dashboard(
    ...
    try:
        ...
        try:
            ...
            # (оптимизация - только последние 100 свечей)
        ...
        # Signals panel - получаем реальные сигналы
        signals: Union[html.Div, list[Any]] = html.Div(
            "Сигналы загружаются..."
        )
        try:
            from components.signals_panel import create_signals_panel
            from web_interface.services.signals_service import get_signals_service
            
            # Получаем сервис сигналов
            signals_service = get_signals_service(
                signal_generator=getattr(app, 'signal_generator', None),
                profit_tracker=getattr(app, 'profit_tracker', None)
            )

            # Получаем реальные сигналы для текущего символа
            real_signals = (
                signals_service.get_all_signals_for_dashboard(symbol)
            )

            # Если есть сигналы, передаем их в компонент
            if real_signals:
                signals_result = create_signals_panel(signals=real_signals)
            else:
                # Если сигналов нет, пытаемся получить для символа
                symbol_signals = signals_service.get_signals_for_symbol(
                    symbol
                )
                if symbol_signals:
                    ...
                    signals_result = create_signals_panel()  # Тестовые
            ...
            logger.warning(
                "Не удалось загрузить компоненты сигналов: %s", str(e)
            )
        except Exception as e:
            logger.error(
                "Ошибка получения сигналов: %s", str(e), exc_info=True
            )
            signals = html.Div(f"Ошибка загрузки сигналов: {str(e)}")

        # Metrics panel - получаем реальные данные
        metrics: Union[html.Div, list[Any]] = html.Div(
            "Метрики загружаются..."
        )
        try:
            from components.metrics_panel import create_metrics_panel
            from web_interface.services.price_service import get_price_service

            # Получаем сервис цен
            price_service = get_price_service(
                data_manager=getattr(app, '_data_manager', None)
            )

            # Получаем актуальные данные тикера
            ticker_data = price_service.get_ticker_data(symbol, 'binance')

            # Получаем метрики торговли из ProfitTracker
            trading_metrics = {}
            try:
                profit_tracker = getattr(app, 'profit_tracker', None)
                if profit_tracker:
                    stats = profit_tracker.get_statistics()
                    trading_metrics = {
                ...
                logger.debug(
                    "Не удалось получить метрики торговли: %s", str(e)
                )

            # Передаем реальные данные в компонент метрик
            if ticker_data:
            ...
            logger.warning(
                "Не удалось загрузить компоненты метрик: %s", str(e)
            )
        except Exception as e:
            logger.error(
                "Ошибка получения метрик: %s", str(e), exc_info=True
            )
            metrics = html.Div(f"Ошибка загрузки метрик: {str(e)}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Оптимизированный TTL
        data_manager = MarketDataManager(cache_ttl_minutes=3)
        alerts = MarketAlerts(data_manager)
    ...
    # Запускаем Telegram бота с системой сигналов
    try:
        ...
        # Создаем генератор сигналов и трекер профитов
        try:
            signal_generator = SignalGenerator(data_manager=data_manager)
            profit_tracker = ProfitTracker(data_manager=data_manager)
            
            # Сохраняем в глобальном контексте приложения для использования в callbacks
            app.signal_generator = signal_generator
            app.profit_tracker = profit_tracker
            
            logger.info("Система сигналов и профитов инициализирована")
        except Exception as e:
            logger.error(
                "Ошибка инициализации системы сигналов: %s",
                str(e), exc_info=True
            )
            signal_generator = None
            profit_tracker = None
            app.signal_generator = None
            app.profit_tracker = None
        ...
        try:
            ...
            if telegram_bot:
                ...
                if telegram_bot.is_running:
                    msg = "✅ Telegram бот успешно запущен: t.me/MaxFlash_bot"
                    logger.info(msg)
                    print(f"\n{msg}")
                else:
                    msg = "⚠️ Telegram бот не запустился (is_running=False)"
                    logger.warning(msg)
                    print(f"\n{msg}. Проверь логи.")
                ...
                logger.error(
                    ...
                    "(get_telegram_bot вернул None)"
                )
                print("\n❌ Не удалось создать Telegram бота. Проверь логи.")
        except Exception as e:
            logger.error(
                "❌ Ошибка создания/запуска Telegram бота: %s",
                str(e), exc_info=True
            )
            print(f"\n❌ Ошибка Telegram бота: {e}")
            print("Проверь логи для подробностей.")
    except ImportError as e:
        logger.error(
            "❌ Не удалось импортировать модули Telegram бота: %s",
            str(e)
        )
        print(f"\n❌ Ошибка импорта: {e}")
        ...
        logger.error(
            "❌ Критическая ошибка Telegram бота: %s",
            str(e), exc_info=True
        )
    ...
    try:
        ...
        try:
            ...
            # Fallback если market_config недоступен
            POPULAR_PAIRS = [
