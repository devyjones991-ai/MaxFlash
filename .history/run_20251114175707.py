...
def install_dependencies():
    ...
    if missing:
        ...
        try:
            ...
            print_status("Попробуйте вручную: pip install -r requirements.txt", "warn")
            return False
    else:
...
def check_port(port=8050):
    ...
    try:
        ...
        if result == 0:
            ...
            print_status("Закройте другое приложение или измените порт", "warn")
            return False
        return True
    except Exception:
        return True
...
def open_browser(url, delay=4):
    ...
    def _open():
        ...
        if wait_for_server(url):
            ...
            print_status("Сервер не запустился, браузер не открыт", "warn")
            print_status(f"Откройте вручную: {url}", "info")
...
def run_dashboard(web_interface_path):
    ...
    # Пробуем запустить приложения в порядке приоритета (production версия)
    app_files = [
    ...
    for app_file in app_files:
        if app_file.exists():
            try:
                ...
                # Запускаем напрямую через exec для правильной работы сервера
                with open(app_file, 'r', encoding='utf-8') as f:
                    code = compile(f.read(), str(app_file), 'exec')
                    # Создаем правильный контекст для выполнения
                    globals_dict = {
                        '__name__': '__main__',
                        '__file__': str(app_file),
                        '__package__': None,
                    }
                    # Копируем важные модули в контекст
