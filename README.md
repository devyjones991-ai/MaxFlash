...
# Или с дополнительными функциями
...
### 2. Запуск приложения (СУПЕР ПРОСТО! 🚀)

**Самый простой способ - один клик:**

**Windows:** Дважды кликните на `run.bat` в корне проекта!
...
### 🌐 Новые возможности: Мульти-рыночный анализ

MaxFlash теперь поддерживает анализ всего криптовалютного рынка:

- **🌐 Market Overview**: Обзор всего рынка с heatmap и таблицей всех пар
- **📊 Multi-View**: Одновременный просмотр до 4 графиков для сравнения
- **🏢 Sector Analysis**: Анализ секторов (DeFi, Layer 1, NFT, Gaming и др.)
- **📈 Корреляции**: Автоматический расчет корреляций между парами
- **🔍 Тренды**: Выявление трендов по всем доступным парам
- **💱 Все биржи**: Поддержка всех бирж через CCXT (100+ бирж)

**Доступные вкладки в интерфейсе:**

- Market Overview - обзор рынка с heatmap и таблицей всех пар
- Multi-View - мульти-экранный просмотр до 4 графиков одновременно
- Sector Analysis - секторный анализ с визуализацией производительности

**Технические возможности:**

- ✅ Асинхронная загрузка данных для производительности
- ✅ Lazy loading для больших списков пар
- ✅ Виртуализация таблиц (показ топ-50, остальные по требованию)
- ✅ Синхронизация таймфреймов между графиками в Multi-View
- ✅ Автоматическое обновление данных (heatmap каждую минуту)
- ✅ Расширенные метрики рынка (BTC доминирование, активные пары)
- ✅ Продвинутое кэширование (Redis опционально для production)
- ✅ Система рыночных алертов (отслеживание резких движений, всплесков объема)
- ✅ Dropdown для быстрого выбора популярных пар в Multi-View
- ✅ Корреляционная матрица секторов

**🚀 Оптимизации производительности:**

- ⚡ **Параллельная загрузка данных** - ThreadPoolExecutor для одновременной загрузки до 10-15 пар (5-10x быстрее)
- ⚡ **Двухуровневое кэширование** - Локальный кэш + MarketCache с поддержкой Redis
- ⚡ **Performance Profiling** - Автоматическое измерение времени выполнения всех операций
- ⚡ **Фоновый мониторинг** - Автоматическое отслеживание рыночных событий каждые 30 секунд
- ⚡ **Оптимизированный AsyncDataLoader** - Использование ThreadPoolExecutor вместо простых потоков (3-5x быстрее)
- 🔴 **WebSocket Real-time обновления** - Мгновенные обновления цен через WebSocket вместо polling (задержка <1 сек вместо 30 сек)

📊 **Результаты**:

- Загрузка 50 пар: ~3-5 секунд вместо 25-30 секунд (**6-10x ускорение**)
- Обновление цен: мгновенно через WebSocket вместо каждые 30 секунд

[Подробнее об оптимизациях →](PERFORMANCE_OPTIMIZATIONS.md) | [WebSocket интеграция →](docs/WEBSOCKET_INTEGRATION.md)

📊 Полная визуализация всех индикаторов в реальном времени!

**Альтернативный способ (через web_interface):**
