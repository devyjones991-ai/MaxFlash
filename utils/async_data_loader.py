...
class AsyncDataLoader:
    ...
    def _load_ticker_worker(self, exchange_id: str, symbol: str, result_dict: dict):
        """
        ...
        try:
            ...
            logger.warning("Ошибка загрузки тикера %s с %s: %s", symbol, exchange_id, str(e))
            result_dict[symbol] = None

    def load_tickers_async(
        self, symbols: list[str], exchange_id: str = "binance", max_workers: int = 15
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_symbol = {executor.submit(load_ticker, symbol): symbol for symbol in symbols}
    ...
    def load_ohlcv_async(
        ...
        def load_ohlcv(symbol: str) -> tuple[str, Any]:
            ...
            try:
                df = self.data_manager.get_ohlcv(symbol, timeframe, limit, exchange_id)
                return symbol, df
            except Exception as e:
                logger.warning("Ошибка загрузки OHLCV %s: %s", symbol, str(e))
                return symbol, None

        # Используем ThreadPoolExecutor для параллельной загрузки
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_symbol = {executor.submit(load_ohlcv, symbol): symbol for symbol in symbols}
