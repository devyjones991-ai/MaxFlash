...
**Мин 24ч:** ${low_24h:,.2f}
            ...
            logger.error(f"Ошибка получения цены для {symbol}: {e!s}")
    ...
    async def alerts_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.args:
            ...
            if not alerts:
                await update.message.reply_text(
                    "🔔 Нет активных уведомлений. Используйте /alerts add <SYMBOL> <PRICE> <above/below>"
                )
                return
        ...
        if action == "add" and len(context.args) >= 4:
            ...
            try:
                ...
                alert = {"symbol": symbol, "price": price, "type": alert_type, "triggered": False}
    ...
    async def search_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.error(f"Ошибка поиска: {e!s}")
            await update.message.reply_text(f"❌ Ошибка: {e!s}")
...
"""
            ...
            logger.error(f"Ошибка получения статистики: {e!s}")
            await update.message.reply_text(f"❌ Ошибка: {e!s}")
    ...
    async def callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif data == "watchlist_clear" and chat_id in self.watchlist:
            self.watchlist[chat_id].clear()
    ...
    def send_notification(self, chat_id: int, message: str, parse_mode: str = "Markdown"):
        ...
        async def _send():
            try:
                await self.application.bot.send_message(chat_id=chat_id, text=message, parse_mode=parse_mode)
            except Exception as e:
                logger.error(f"Ошибка отправки сообщения в Telegram: {e!s}")
...
**Тип:** {alert_type}
                    ...
                    logger.error(f"Ошибка проверки уведомления для {symbol}: {e!s}")
...
**Изменение 24ч:** {change_24h:+.2f}%
    ...
    def _check_alerts_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.error(f"Ошибка проверки уведомлений: {e!s}")
                time.sleep(10)
    ...
    def _run_bot(self):
        ...
        async def post_init(app: Application):
            ...
            alerts_thread = threading.Thread(target=self._check_alerts_loop, daemon=True, name="TelegramBotAlerts")
            alerts_thread.start()
        ...
        def run():
            try:
                ...
                logger.error(f"Ошибка запуска Telegram бота: {e!s}", exc_info=True)
                self.is_running = False
    ...
    def stop(self):
        ...
        self.is_running = False
        if self.application:
            asyncio.run_coroutine_threadsafe(self.application.stop(), self.application.bot._loop)
    ...
    if _telegram_bot_instance is None and token:
        try:
            _telegram_bot_instance = TelegramBot(
                token=token, data_manager=data_manager, alerts=alerts, allowed_chat_ids=allowed_chat_ids
            )
        except Exception as e:
            logger.error(f"Ошибка создания Telegram бота: {e!s}")
            return None
