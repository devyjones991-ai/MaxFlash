...
class PriceAnomalyDetector:
    ...
    def _detect_z_score_anomalies(self, dataframe: pd.DataFrame) -> list[dict]:
        ...
        dataframe["mean_change"] = dataframe["price_change"].rolling(window=self.window_size, min_periods=1).mean()
        dataframe["std_change"] = dataframe["price_change"].rolling(window=self.window_size, min_periods=1).std()
    ...
    def _detect_volume_spikes(self, dataframe: pd.DataFrame) -> list[dict]:
        ...
        dataframe["avg_volume"] = dataframe["volume"].rolling(window=self.window_size, min_periods=1).mean()
    ...
    def _detect_price_spikes(self, dataframe: pd.DataFrame) -> list[dict]:
        ...
        dataframe["avg_range"] = dataframe["high_low_range"].rolling(window=self.window_size, min_periods=1).mean()
    ...
    def get_anomaly_summary(self, dataframe: pd.DataFrame) -> dict:
        ...
        if not anomalies:
            return {"total_anomalies": 0, "high_severity": 0, "medium_severity": 0, "by_type": {}}
