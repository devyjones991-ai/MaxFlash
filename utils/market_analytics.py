...
class MarketAnalytics:
    ...
    def calculate_correlations(
        self, pairs: list[str], timeframe: str = "1h", period_days: int = 30, exchange_id: str = "binance"
    ) -> pd.DataFrame:
        """
        ...
        for pair in pairs:
            try:
                df = self.data_manager.get_ohlcv(pair, timeframe, limit, exchange_id)
                if df is not None and len(df) >= CORRELATION_CONFIG["min_data_points"]:
                    price_data[pair] = df["close"]
            except Exception as e:
                logger.warning("Ошибка загрузки данных для %s: %s", pair, str(e))
        ...
        logger.info("Рассчитана корреляционная матрица для %s пар", len(pairs))
        return correlation_matrix

    def detect_trends(
        self, pair: str, timeframe: str = "4h", period_days: int = 7, exchange_id: str = "binance"
    ) -> dict[str, Any]:
        """
    ...
    def find_opportunities(
        self, pairs: Optional[list[str]] = None, min_correlation: float = 0.7, exchange_id: str = "binance"
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            trends = {}
            for pair in pairs:
                trend_info = self.detect_trends(pair, timeframe="4h", period_days=7, exchange_id=exchange_id)
            ...
            for i, pair1 in enumerate(pairs):
                ...
                for pair2 in pairs[i + 1 :]:
                    ...
                    if pair1 in correlation_matrix.index and pair2 in correlation_matrix.columns:
                        ...
                        if abs(corr) > min_correlation and trend1["trend"] != trend2["trend"]:
                        ...
                        if (
                            abs(corr) > min_correlation
                            and trend1["trend"] == trend2["trend"]
                            and trend1["strength"] > 0.7
                        ):
                            opportunities.append(
                                {
                                    "type": "strong_trend",
                                    ...
                                    "description": (
                                        f"Сильный {trend1['trend']} тренд в {pair1} и {pair2} (корреляция: {corr:.2f})"
                                    ),
                                }
                            )
    ...
    def get_sector_performance(self, sector: str, exchange_id: str = "binance") -> dict[str, Any]:
        """
        ...
        trends = []
        for pair in pairs:
            trend_info = self.detect_trends(pair, timeframe="1d", period_days=7, exchange_id=exchange_id)
            if trend_info.get("trend") != "unknown":
                trends.append({"pair": pair, **trend_info})
