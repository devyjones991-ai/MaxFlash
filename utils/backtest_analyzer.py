...
class BacktestAnalyzer:
    ...
    def calculate_statistics(
        ...
        # Returns
        total_return = (equity_curve.iloc[-1] - initial_capital) / initial_capital * 100 if len(equity_curve) > 0 else 0.0
        ...
        if 'profit_abs' in trades.columns or 'profit' in trades.columns:
            profit_col = 'profit_abs' if 'profit_abs' in trades.columns else 'profit'
            stats_dict['avg_win'] = trades[trades[profit_col] > 0][profit_col].mean() if len(trades[trades[profit_col] > 0]) > 0 else 0.0
            stats_dict['avg_loss'] = trades[trades[profit_col] < 0][profit_col].mean() if len(trades[trades[profit_col] < 0]) > 0 else 0.0
        ...
        if stats_dict['total_trades'] > 0:
            stats_dict['expectancy'] = (
                (stats_dict['win_rate'] / 100) * stats_dict['avg_win'] +
                ((1 - stats_dict['win_rate'] / 100)) * stats_dict['avg_loss']
            )
        else:
    ...
    def walk_forward_analysis(
        ...
        for period_name in trades_by_period.keys():
            period_stats = self.calculate_statistics(
                trades_by_period[period_name],
                equity_by_period[period_name],
                returns_by_period[period_name]
            )
    ...
    def print_performance_report(
        ...
        print(f"  Average Return: {stats.get('avg_return', 0)*100:.4f}%")
