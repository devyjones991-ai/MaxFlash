...
class MarketStructureAnalyzer:
    ...
    def _detect_break_of_structure(
        ...
        for i in range(len(dataframe)):
            ...
            if last_swing_high is not None:
                if dataframe.iloc[i]['close'] > last_swing_high:
                    bos.iloc[i] = True
            ...
            if last_swing_low is not None:
                if dataframe.iloc[i]['close'] < last_swing_low:
                    bos.iloc[i] = True
    ...
    def _detect_change_of_character(
        ...
        for i in range(len(dataframe)):
            ...
            if len(recent_swing_lows) >= 2:
                if recent_swing_lows[-1] > recent_swing_lows[-2]:
                    choch.iloc[i] = True
                    choch_type.iloc[i] = 'bullish'
            
            # Check for bearish ChoCH (lower high)
            if len(recent_swing_highs) >= 2:
                if recent_swing_highs[-1] < recent_swing_highs[-2]:
                    choch.iloc[i] = True
    ...
    def _determine_trend(
        ...
        recent_highs = [h for h in swing_highs.dropna().tail(3) if pd.notna(h)]
        recent_lows = [l for l in swing_lows.dropna().tail(3) if pd.notna(l)]
        ...
        for i in range(len(dataframe)):
            ...
            if len(current_highs) >= 2 and len(current_lows) >= 2:
                ...
                if (current_highs[-1] > current_highs[-2] and
                    current_lows[-1] > current_lows[-2]):
                    trend.iloc[i] = 'bullish'
                
                # Bearish trend: lower highs and lower lows
                elif (current_highs[-1] < current_highs[-2] and
                      current_lows[-1] < current_lows[-2]):
    ...
    def get_market_structure_summary(
        self,
        dataframe: pd.DataFrame
    ) -> Dict:
        """
