...
class VolumeProfileCalculator:
    ...
    def _calculate_profile_for_period(
        self,
        dataframe: pd.DataFrame
    ) -> Dict:
        """
        ...
        hvn = [bin_centers[i] for i in range(self.bins) 
               if volume_by_bin[i] >= hvn_threshold]
        lvn = [bin_centers[i] for i in range(self.bins) 
               if volume_by_bin[i] <= lvn_threshold and volume_by_bin[i] > 0]
