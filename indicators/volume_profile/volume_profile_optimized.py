...
from utils.performance import global_cache, global_profiler, vectorized_volume_distribution
...
class VolumeProfileCalculatorOptimized:
    ...
    def _calculate_profile_for_period_vectorized(self, dataframe: pd.DataFrame) -> dict:
        """
        ...
        volume_by_bin = vectorized_volume_distribution(
            dataframe["low"].values, dataframe["high"].values, dataframe["volume"].values, bin_edges, self.bins
        )
        ...
        val, vah = self._calculate_value_area_vectorized(volume_by_bin, bin_centers, poc_bin, target_volume)
