...
# Supported exchanges (NO BINANCE - IP banned)
EXCHANGES = {
    'kucoin': {'name': 'KuCoin', 'icon': '🟢', 'fee': 0.1},
...
# Exchange instances (multiple exchanges - NO BINANCE due to IP ban)
exchanges = {
...
exchange = exchanges['kucoin']  # Default exchange (NOT Binance!)
...
def get_market_data():
    ...
    Falls back to direct CCXT API call if cache is missing/stale.
    """
    try:
        ...
        # 2. Fallback: Use CCXT directly (KuCoin, Bybit, OKX - NOT Binance!)
        logger.warning("Cache missing or stale, using CCXT fallback")

        # Our TOP 50 pairs to fetch
        top_pairs = [
            "BTC/USDT", "ETH/USDT", "BNB/USDT", "XRP/USDT", "SOL/USDT",
            ...
            "ATOM/USDT", "FIL/USDT", "HBAR/USDT", "ARB/USDT", "OP/USDT",
            "INJ/USDT", "SUI/USDT", "VET/USDT", "ALGO/USDT", "FTM/USDT",
            "AAVE/USDT", "GRT/USDT", "THETA/USDT", "XLM/USDT", "RENDER/USDT",
            "IMX/USDT", "MKR/USDT", "SAND/USDT", "MANA/USDT", "AXS/USDT",
            "PEPE/USDT", "FLOKI/USDT", "WIF/USDT", "BONK/USDT", "WLD/USDT",
            "RUNE/USDT", "EGLD/USDT", "XTZ/USDT", "FLOW/USDT", "EOS/USDT",
        ]

        data = []
        # Try exchanges in order: KuCoin -> OKX -> Bybit
        for ex_name in ['kucoin', 'okx', 'bybit']:
            if data:  # Already got data
                break
            try:
                ...
                for pair in top_pairs:
                    ...
                    if change > 3:
                        signal, signal_score = '🟢 BUY', min(70 + int(change * 2), 95)
                    elif change < -3:
                        signal, signal_score = '🔴 SELL', max(30 - int(abs(change) * 2), 5)
                    else:
                    ...
                    data.append({
                        ...
                        'reasons': [f"24h: {change:+.1f}%", f"Source: {ex_name}"],
                    })

                if data:
                    logger.info(f"Got {len(data)} pairs from {ex_name}")
                    break

            except Exception as ex_error:
                logger.warning(f"CCXT {ex_name} failed: {ex_error}")
                continue

        if data:
            return pd.DataFrame(data)

        logger.error("All CCXT exchanges failed!")
        return pd.DataFrame()
...
def update_market_data(n1, n2, signal_filter):
    ...
    if df.empty:
        logger.warning("update_market_data: DataFrame is empty!")
        return [], [], "0", "0", "-", "-"
    ...
    # Count signals (before filtering) - signals can be '🟢 BUY' or '🔴 SELL'
    ...
    # Apply signal filter - signals can be '🟢 BUY' or '🔴 SELL'
    if signal_filter == "BUY":
        df = df[df['signal'].str.contains('BUY', na=False)]
    elif signal_filter == "SELL":
        df = df[df['signal'].str.contains('SELL', na=False)]
    ...
    logger.info(f"update_market_data: {len(df)} coins, BUY={buy_count}, SELL={sell_count}")
...
def update_signal_rows(market_data):
    ...
    for idx, row in enumerate(market_data):
        ...
        # Check if signal contains BUY or SELL (can be '🟢 BUY' or '🔴 SELL')
        is_buy = 'BUY' in str(signal)
        is_sell = 'SELL' in str(signal)
        ...
        className=f"btn btn-sm {'btn-success' if is_buy else 'btn-danger'} text-white px-2 py-1 me-1 mb-1",
...
def update_chart(selected_rows, timeframe, exchange_id, n_fast, market_data):
    ...
    if not selected_rows or not market_data:
        ...
        return empty_fig, "Select coin", html.Div("Выберите монету из таблицы", className="text-muted text-center")
    ...
    try:
        ...
        if df.empty:
            ...
            return empty_fig, symbol, html.Div("No data")
        ...
        # Get exchange info
        ex_info = EXCHANGES.get(exchange_id, EXCHANGES['kucoin'])
        ...
        try:
            ...
            selected_exchange = exchanges.get(exchange_id, exchanges['kucoin'])
        ...
        return error_fig, "Error", html.Div(f"Error: {e}")
...
def update_ecosystem_overview(n_intervals, market_data):
    ...
    try:
        # Count crypto coins (total in market_data store)
        crypto_count = 0
        if market_data:
            crypto_count = len(market_data)
