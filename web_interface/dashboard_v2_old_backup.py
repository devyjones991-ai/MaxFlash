...
Senior-level trading dashboard with TOP 20 coins, responsive design, and improved signal detection.
"""
...
TOP_50_PAIRS = [
    # TOP 20 - Verified on Binance
    "BTC/USDT", "ETH/USDT", "BNB/USDT", "SOL/USDT", "XRP/USDT",
    "DOGE/USDT", "ADA/USDT", "AVAX/USDT", "DOT/USDT", "MATIC/USDT",
    "LINK/USDT", "UNI/USDT", "PEPE/USDT", "AAVE/USDT", "SUI/USDT",
    "LTC/USDT", "ARB/USDT", "OP/USDT", "INJ/USDT", "SHIB/USDT",
]
...
def get_market_data():
    """Fetch market data for TOP 20 pairs with enhanced signals."""
    try:
...
def create_header():
    ...
    return dbc.Navbar(
        dbc.Container([
            ...
            dbc.Row([
                dbc.Col(
                    dbc.Badge("TOP 20", color="success", className="me-1 d-none d-md-inline"),
                    width="auto"
                ),
                dbc.Col(
...
app.layout = html.Div([
    ...
    dcc.Interval(id="interval-slow", interval=20*1000, n_intervals=0),  # 20s - обновление таблицы TOP 20
...
def update_market_data(n1, n2, signal_filter):
    ...
    logger.info(f"Signal counts: BUY={buy_count}, SELL={sell_count}, HOLD={len(df) - buy_count - sell_count}")
    if buy_count > 0 or sell_count > 0:
        active_signals = df[df['signal'] != 'HOLD'][['symbol', 'signal', 'signal_score']].to_dict('records')
...
def update_header_time(n):
    """Update header time (next to TOP 20) in Moscow time."""
