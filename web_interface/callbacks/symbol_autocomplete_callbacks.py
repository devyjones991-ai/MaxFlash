...
def register_symbol_autocomplete_callbacks(app, data_manager=None):
    ...
    @app.callback(
        ...
        [
            Input(f"quick-{pair.replace('/', '-')}", "n_clicks")
            for pair in [
                "BTC/USDT",
                "ETH/USDT",
                "SOL/USDT",
    ...
    @app.callback(
        [Output("symbol-input-suggestions", "children"), Output("symbol-input-suggestions", "style")],
    ...
    def update_suggestions(symbol_value):
        ...
        suggestions = [pair for pair in POPULAR_PAIRS[:100] if query_upper in pair.upper()][:10]
