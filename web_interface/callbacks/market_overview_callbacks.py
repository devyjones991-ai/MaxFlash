...
try:
    from components.market_overview import create_crypto_heatmap, create_pairs_table
except ImportError:
    from web_interface.components.market_overview import create_crypto_heatmap, create_pairs_table
...
def register_market_overview_callbacks(app, data_manager=None):
    ...
    def sort_pairs_table(_pair, _price, _change, _volume, stored_data, search_query):
        ...
        if stored_data and "tickers" in stored_data:
            ...
            tickers = async_loader.load_tickers_async(POPULAR_PAIRS[:100], "binance", max_workers=10)
    ...
    @app.callback(
        ...
        [Input("pair-search-btn", "n_clicks"), Input("pair-search-input", "n_submit")],
        [State("pair-search-input", "value"), State("pairs-data-store", "data")],
    )
        ...
        try:
            ...
            if stored_data and "tickers" in stored_data:
                ...
                tickers = async_loader.load_tickers_async(
                    POPULAR_PAIRS[:50],
                    "binance",
                    max_workers=5,  # Уменьшено для скорости
                )
            ...
            return html.Div([dbc.Alert(f"Ошибка загрузки данных: {e!s}", color="danger")])

    @app.callback(Output("crypto-heatmap", "figure"), [Input("heatmap-interval", "n_intervals")])
    def update_heatmap(_n_intervals):
        ...
        try:
            ...
            fig = go.Figure()
            fig.add_annotation(
                text=f"Ошибка обновления: {e!s}", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False
            )
            fig.update_layout(template="plotly_dark")
            return fig

    @app.callback(Output("market-alerts-list", "children"), [Input("alerts-interval", "n_intervals")])
    def update_alerts(_n_intervals):
        ...
        try:
            ...
            if not recent_alerts:
                return html.Div([dbc.Alert("Нет активных алертов", color="info")])

            alert_items = []
            for alert in recent_alerts:
                severity_color = {"info": "info", "warning": "warning", "critical": "danger"}.get(
                    alert["severity"], "info"
                )
            ...
            return html.Div([dbc.Alert(f"Ошибка загрузки алертов: {e!s}", color="danger")])
