...
def register_price_alerts_callbacks(app, data_manager=None):
    ...
    @app.callback(
        ...
        [Input("alert-add-btn", "n_clicks"), Input({"type": "alert-remove", "symbol": "*"}, "n_clicks")],
        [
    ...
    @app.callback(
        ...
        [Input("alerts-store", "data"), Input("alerts-check-interval", "n_intervals")],
    )
    def update_alerts_display(stored_data, _n_intervals):
        ...
        if not stored_data or "alerts" not in stored_data:
            return html.Div([dbc.Alert("Нет активных уведомлений", color="info", className="mb-0")])
        ...
        for alert in alerts:
            ...
            try:
                ...
                # Проверяем условие
                if (alert_type == "above" and current_price >= price) or (
                    alert_type == "below" and current_price <= price
                ):
                    triggered = True
                                ...
                                [
                                    dbc.Col(
                                        [
                                            ...
                                            html.Br(),
                                            html.Small(
                                                f"Уведомить когда {type_text} ${price:,.2f}", className="text-muted"
                                            ),
                                            html.Br(),
                                            html.Small(f"Текущая: ${current_price:,.2f}", className="text-info"),
                                        ],
                                        width=10,
                                    ),
                                    dbc.Col(
                ...
                items.append(
                    dbc.Alert(
                        [html.Strong(symbol), html.Br(), html.Small("Ошибка получения данных", className="text-muted")],
                        color="warning",
                        className="mb-2",
                    )
                )
    ...
    @app.callback(
        [Output("alert-symbol-input", "value"), Output("alert-price-input", "value")],
        [Input("alert-add-btn", "n_clicks")],
        [State("alert-symbol-input", "value"), State("alert-price-input", "value")],
    )
