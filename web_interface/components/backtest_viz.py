...
def create_backtest_results(backtest_data: Optional[dict] = None) -> list:
    ...
    equity_fig.add_hline(y=initial, line_dash="dash", line_color="gray", annotation_text=f"Initial: ${initial:,.2f}")
    ...
    metrics_table = dash_table.DataTable(
        ...
        style_cell={"backgroundColor": "rgb(30, 30, 30)", "color": "white", "textAlign": "left", "fontFamily": "Arial"},
        style_header={"backgroundColor": "rgb(60, 60, 60)", "fontWeight": "bold"},
    )

    return [
        dbc.Row(
            [
                dbc.Col([dcc.Graph(figure=equity_fig)], width=8),
                dbc.Col([dbc.Card([dbc.CardHeader("Performance Metrics"), dbc.CardBody([metrics_table])])], width=4),
            ]
        )
    ]
