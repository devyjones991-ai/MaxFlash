...
def create_modern_price_card(symbol: str, price: float, change: float, volume: str = "") -> html.Div:
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Div(f"${price:,.2f}", className="price-value"),
            html.Div(
                [change_emoji, f"{abs(change):.2f}%"],
...
def create_modern_signal_item(signal: dict) -> html.Div:
    ...
    return html.Div(
        ...
        children=[
            html.Div(signal_type, className=f"signal-type {'long' if is_long else 'short'}"),
            html.Div(
                f"{signal.get('symbol', 'N/A')} @ ${signal.get('price', 0):,.2f}",
                style={"fontWeight": "600", "marginBottom": "0.25rem", "color": "var(--text-primary)"},
            ),
            html.Div(
                [
...
def create_modern_candlestick_chart(
    dataframe: pd.DataFrame, show_order_blocks: bool = True, show_fvg: bool = True
) -> go.Figure:
    """
    ...
    # Volume bars
    colors = ["#0ECB81" if close >= open else "#F6465D" for close, open in zip(dataframe["close"], dataframe["open"])]
    ...
    # Стилизация осей
    fig.update_xaxes(gridcolor="rgba(255, 255, 255, 0.1)", showgrid=True, zeroline=False, row=2, col=1)

    fig.update_yaxes(gridcolor="rgba(255, 255, 255, 0.1)", showgrid=True, zeroline=False, row=1, col=1)

    fig.update_yaxes(gridcolor="rgba(255, 255, 255, 0.1)", showgrid=True, zeroline=False, row=2, col=1)
            ...
            html.Div(
                [
                    ...
                    html.Span(
                        ...
                        style={"color": pnl_color, "fontWeight": "700", "fontSize": "1.5rem", "marginLeft": "1rem"},
                    ),
                ],
