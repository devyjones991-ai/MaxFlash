...
def create_metrics_panel(metrics: Dict = None) -> list:
    """
    ...
    metric_items = [
        ("Win Rate", f"{metrics.get('win_rate', 0):.1f}%", 
         "success" if metrics.get('win_rate', 0) > 50 else "warning"),
    ...
    for label, value, color in metric_items:
        children.append(
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Badge(value, color=color, className="float-end")
                ], width=4)
            ], className="mb-2")
        )
    ...
    children.append(
        html.Div([
            ...
            html.Span(f"{metrics.get('total_trades', 0)}", className="float-end")
        ], className="mb-2")
    )
    ...
    children.append(
        html.Div([
            html.Strong("Avg Win: "),
            html.Span(f"${metrics.get('avg_win', 0):.2f}", 
    ...
    children.append(
        html.Div([
            ...
            html.Span(f"${metrics.get('avg_loss', 0):.2f}",
