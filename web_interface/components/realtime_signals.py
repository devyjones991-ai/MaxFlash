...
def create_realtime_signals_panel(signals: Optional[list[dict]] = None) -> list:
    """
    ...
    if not signals:
        return [dbc.Alert("No real-time signals", color="info")]
            ...
            [
                    ...
                    [
                        dbc.Row(
                            [
                                ...
                                dbc.Col([dbc.Badge(pnl_text, color=pnl_color, className="float-end")], width="auto"),
                            ]
                        )
                    ]
                ),
                dbc.CardBody(
                    [
                        ...
                        html.Hr(),
                        html.Div(
                            [html.Strong("Confluence: "), html.Span(f"{sig['confluence']}/5", className="float-end")],
                            className="mb-2",
                        ),
                        html.Div(
                            [
                        ...
                        html.Small(f"Time: {sig['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}", className="text-muted"),
                    ]
                ),
            ],
            className="mb-3",
