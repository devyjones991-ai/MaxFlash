...
def create_footprint_chart(dataframe: Optional[pd.DataFrame] = None) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(x=dataframe.index, y=dataframe["fp_buy_volume"], name="Buy Volume", marker_color="green", opacity=0.7),
        row=2,
        col=1,
    )

    fig.add_trace(
        go.Bar(x=dataframe.index, y=-dataframe["fp_sell_volume"], name="Sell Volume", marker_color="red", opacity=0.7),
        row=2,
        col=1,
    )
    ...
    # Обновление layout
    fig.update_layout(
        template="plotly_dark", height=600, showlegend=True, xaxis_rangeslider_visible=False, hovermode="x unified"
    )
