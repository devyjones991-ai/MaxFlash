...
class FreqtradeClient:
    ...
    def __init__(
        self, base_url: str = "http://localhost:8080", username: Optional[str] = None, password: Optional[str] = None
    ):
        """
    ...
    def get_trades(self, limit: int = 50) -> list[dict]:
        ...
        """
        try:
            response = self.session.get(f"{self.base_url}/api/v1/trades", params={"limit": limit})
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
