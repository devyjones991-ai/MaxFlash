    ...
    async def signals_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(filtered_signals) > 5:
            await update.message.reply_text(
                f"Показано 5 из {len(filtered_signals)} сигналов. "
                "Подпишитесь на Pro/Alpha для большего: /subscribe"
            )
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data.startswith("subscribe_"):
            ...
            await query.edit_message_text(
                f"Подписка {rating} - функция в разработке. "
                "Свяжитесь с поддержкой: @MaxFlashSupport"
            )
