...
class SMCFootprintStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.fvg_detector = FairValueGapDetector(min_size_pct=self.fvg_min_size_pct, max_age_bars=self.fvg_max_age_bars)

        self.market_structure = MarketStructureAnalyzer()

        self.vp_calculator = VolumeProfileCalculator(value_area_percent=self.vp_value_area_percent)

        self.mp_calculator = MarketProfileCalculator(period=self.mp_period)
        ...
        self.confluence_calc = ConfluenceCalculator(min_signals=self.min_confluence_signals)

        self.risk_manager = RiskManager(risk_per_trade=self.risk_per_trade)
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # Volume Profile
        dataframe = self.vp_calculator.calculate_volume_profile(dataframe, period=self.vp_period)
    ...
    def populate_indicators_1d(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe = self.vp_calculator.calculate_volume_profile(
            dataframe,
            period=50,  # Longer period for daily
        )
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        for i in range(self.startup_candle_count, len(dataframe)):
            ...
            # Check confluence
            confluence_score = self._calculate_confluence(macro_analysis, intermediate_analysis, micro_analysis)

            # Entry conditions
            if confluence_score >= self.min_confluence_signals:
                # Long entry
                if (
                    macro_analysis.get("trend") in ["bullish", "range"]
                    and macro_analysis.get("in_order_block")
                    and macro_analysis.get("order_block_type") == "bullish"
                    and intermediate_analysis.get("in_value_area", True)
                    and micro_analysis.get("delta_alignment") == "bullish"
                    and micro_analysis.get("absorption_detected", False)
                ):
                    dataframe.loc[dataframe.index[i], "enter_long"] = 1

                # Short entry
                elif (
                    macro_analysis.get("trend") in ["bearish", "range"]
                    and macro_analysis.get("in_order_block")
                    and macro_analysis.get("order_block_type") == "bearish"
                    and intermediate_analysis.get("in_value_area", True)
                    and micro_analysis.get("delta_alignment") == "bearish"
                    and micro_analysis.get("absorption_detected", False)
                ):
    ...
    def custom_stoploss(
        self, pair: str, trade: "Trade", current_time, current_rate: float, current_profit: float, **kwargs
    ) -> float:
        """
    ...
    def _analyze_macro_level(self, current_data: pd.Series, macro_data: pd.Series | None) -> dict:
        ...
        if pd.notna(macro_data.get("ob_bullish_low")) and (
            macro_data["ob_bullish_low"] <= current_price <= macro_data["ob_bullish_high"]
        ):
            analysis["in_order_block"] = True
            analysis["order_block_type"] = "bullish"

        if pd.notna(macro_data.get("ob_bearish_low")) and (
            macro_data["ob_bearish_low"] <= current_price <= macro_data["ob_bearish_high"]
        ):
            analysis["in_order_block"] = True
            analysis["order_block_type"] = "bearish"

        # Check FVG
        if pd.notna(macro_data.get("fvg_bullish_low")) and (
            macro_data["fvg_bullish_low"] <= current_price <= macro_data["fvg_bullish_high"]
        ):
    ...
    def _analyze_intermediate_level(self, current_data: pd.Series, intermediate_data: pd.Series | None) -> dict:
    ...
    def _calculate_confluence(self, macro: dict, intermediate: dict, micro: dict) -> int:
