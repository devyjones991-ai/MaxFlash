...
class TestVolumeProfile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.calculator = VolumeProfileCalculator(bins=70, value_area_percent=0.70)
    ...
    def test_calculate_volume_profile(self):
        ...
        assert "vp_total_volume" in result.columns
    ...
    def test_poc_calculation(self):
        ...
        if pd.notna(poc):
            assert poc >= price_min
            assert poc <= price_max
    ...
    def test_hvn_lvn_detection(self):
        ...
        assert "hvn" in summary
        assert "lvn" in summary
        assert isinstance(summary["hvn"], list)
        assert isinstance(summary["lvn"], list)

    def test_rolling_period(self):
        """Test Volume Profile with rolling period."""
        result = self.calculator.calculate_volume_profile(self.dataframe, period=20)
