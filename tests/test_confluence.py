...
class TestConfluenceCalculator(unittest.TestCase):
    ...
    def test_find_confluence_zones(self):
        ...
        order_blocks = [{"high": 105, "low": 100, "type": "bullish"}]

        fvgs = [{"high": 104, "low": 102, "type": "bullish", "strength": 1.0}]

        volume_profile = {"poc": 103, "hvn": [102, 104], "lvn": []}

        market_profile = {"vah": 104, "val": 102, "poc": 103}

        zones = self.calculator.find_confluence_zones(order_blocks, fvgs, volume_profile, market_profile)

        assert isinstance(zones, list)

    def test_is_price_in_zone(self):
        """Test price in zone check."""
        zone = {"high": 105, "low": 100, "level": 102.5}

        # Price in zone
        assert self.calculator.is_price_in_zone(102, zone)

        # Price below zone
        assert not self.calculator.is_price_in_zone(99, zone)

        # Price above zone
        assert not self.calculator.is_price_in_zone(106, zone)
    ...
    def test_min_signals_threshold(self):
        ...
        volume_profile = {"poc": 103, "hvn": [], "lvn": []}

        zones = calculator.find_confluence_zones(order_blocks, fvgs, volume_profile)
