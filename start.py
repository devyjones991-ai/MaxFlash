...
if __name__ == "__main__":
    # Проверяем, запущен ли через pythonw (без консоли)
    ...
    try:
        ...
        if not is_silent:
            ...
            # В режиме без консоли логируем в файл
            import logging
            logging.basicConfig(
                filename='logs/startup_error.log',
                level=logging.ERROR
            )
            logging.error("Ошибка запуска: %s", str(e), exc_info=True)
        sys.exit(1)
